...
router.post("/predict", upload.single("file"), async (req, res) => {
    try {
        ...
        // ✅ Make the FastAPI request for Tuberculosis Prediction
        const response = await axios.post("https://medical-ai-production.up.railway.app/predict/tuberculosis/", formData, {
            headers: {
                ...formData.getHeaders(), // ✅ Set correct headers for multipart
            },
        });

        console.log("✅ Tuberculosis Prediction received:", response.data);
        ...
        console.error("❌ Tuberculosis Prediction Error:", error);
        res.status(500).json({
