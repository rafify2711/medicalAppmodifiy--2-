...
router.post("/predict", upload.single("file"), async (req, res) => {
    try {
        ...
        const response = await axios.post("https://medical-ai-production.up.railway.app/predict/eye-diseases/", formData, {
            headers: {
                ...formData.getHeaders(), // ✅ Set correct headers for multipart
            },
        });

        console.log("✅ eye_diseases Prediction received:", response.data);
        ...
        console.error("❌ eye_diseases Prediction Error:", error);
        res.status(500).json({
