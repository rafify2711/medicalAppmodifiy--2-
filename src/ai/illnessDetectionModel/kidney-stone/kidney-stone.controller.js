...
router.post("/predict", upload.single("file"), async (req, res) => {
    try {
        ...
        const response = await axios.post("https://medical-ai-production.up.railway.app/predict/kidney-stone/", formData, {
            headers: {
                ...formData.getHeaders(), // ✅ Set correct headers for multipart
            },
        });

        console.log("✅ Kidney Stone Prediction received:", response.data);
